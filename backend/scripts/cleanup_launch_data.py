...
# A few WIPE tables hold rows that legitimately belong to preserved accounts and
# must survive. `employees` is wiped, but the verified owner / accountant have
# their OWN employee records (users.employee_id points at them); deleting those
# would orphan a preserved account and break its attendance / schedule / Telegram
# links. Restrict such deletes to rows NOT referenced by a surviving user.
WIPE_WHERE = {
    "employees": ("id NOT IN "
...
def _count(db, table, where=None):
    ...
    try:
        ...
        # `where` is only ever a fixed clause from WIPE_WHERE (never user input).
        sql = f'SELECT COUNT(*) FROM "{table}"' + (f" WHERE {where}" if where else "")  # nosec B608
        return db.execute(text(sql)).scalar() or 0  # nosec B608
...
def inventory(db):
    ...
    # For WIPE tables the reported count is what would ACTUALLY be deleted (i.e.
    # honouring any WIPE_WHERE restriction). Rows kept back by such a restriction —
    # e.g. a surviving account's own employee record — are reported separately so
    # the operator can confirm no preserved account is touched.
    kept_by_reference = []
    for t in wipe:
        clause = WIPE_WHERE.get(t)
        if clause:
            total = _count(db, t)
            deletable = _count(db, t, clause)
            if total is not None and deletable is not None and total > deletable:
                kept_by_reference.append((t, total - deletable))
    return {
        "wipe": [(t, _count(db, t, WIPE_WHERE.get(t))) for t in wipe],
        "kept_by_reference": kept_by_reference,
...
def run(db, verified_owner_id, apply=False, backup_confirmed=False):
    ...
    for t in [x for x in WIPE_ORDER if x in _all_tables(db)]:
        clause = WIPE_WHERE.get(t)
        before = _count(db, t, clause)
        # t is from the fixed WIPE_ORDER allowlist intersected with live tables;
        # clause (if any) is a fixed WIPE_WHERE restriction, never user input.
        sql = f'DELETE FROM "{t}"' + (f" WHERE {clause}" if clause else "")  # nosec B608
    ...
    verify_owner_can_login(db, verified_owner_id)
    if getattr(owner, "employee_id", None):
        surviving = db.execute(
            text('SELECT COUNT(*) FROM "employees" WHERE id = :eid'),
            {"eid": owner.employee_id}).scalar() or 0
        if surviving == 0:
            raise ValueError("Post-cleanup check failed: verified owner's employee "
                             "record would be deleted — rolling back.")
    db.commit()
...
def _print_inventory(inv):
    ...
    if inv.get("kept_by_reference"):
        print("\n=== KEPT BACK inside a WIPE table (belongs to a surviving account) ===")
        line(inv["kept_by_reference"])
